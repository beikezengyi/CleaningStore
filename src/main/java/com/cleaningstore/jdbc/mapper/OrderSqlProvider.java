...
public class OrderSqlProvider {
	...
	public String selectOrderByCond(SelectOrderCondition cond) {
		...
		sb.append("select " + " t1.ordernumber, "
				...
				+ " t2.otherName," + " t2.washcount||t2.washunit||t6.thingname as washCountWashUnitThingNumber,"
				+ " t7.washWayName," + " to_char(t2.expectedDate,'yyyy-mm-dd hh24:mi') as expectedDate,"
				+ " to_char(t2.realDate,'yyyy-mm-dd hh24:mi') as realDate," + " t2.thingPrice,"
				+ " t2.managementNumber," + " case when t2.visittogetorder and t2.visittoputorder then '取件送件' "
				...
				+ " left outer join orderdetailstable as t2" + " on(t1.cleanthingnumber=t2.cleanthingnumber)"
		...
		if (isExist(cond.getCreateDateStart())) {
			if (multiCount > 0) {
				sb.append(" and to_char(t2.createDate,'yyyy-mm-dd') >= " + Squ(cond.getCreateDateStart()));
			} else {
				sb.append(" where to_char(t2.createDate,'yyyy-mm-dd') >= " + Squ(cond.getCreateDateStart()));
			}
			multiCount++;
		}
		if (isExist(cond.getCreateDateEnd())) {
			if (multiCount > 0) {
				sb.append(" and to_char(t2.createDate,'yyyy-mm-dd') <= " + Squ(cond.getCreateDateEnd()));
			} else {
				sb.append(" where to_char(t2.createDate,'yyyy-mm-dd') <= " + Squ(cond.getCreateDateEnd()));
			}
			multiCount++;
		}
		...
		if (isExist(cond.getCreateOrderDateStart())) {
			if (multiCount > 0) {
				sb.append(" and to_char(t1.ordercreatedate,'yyyy-mm-dd') >= " + Squ(cond.getCreateOrderDateStart()));
			} else {
				sb.append(" where to_char(t1.ordercreatedate,'yyyy-mm-dd') >= " + Squ(cond.getCreateOrderDateStart()));
			}
			multiCount++;
		}
		if (isExist(cond.getCreateOrderDateEnd())) {
			if (multiCount > 0) {
				sb.append(" and to_char(t1.ordercreatedate,'yyyy-mm-dd') <= " + Squ(cond.getCreateOrderDateEnd()));
			} else {
				sb.append(" where to_char(t1.ordercreatedate,'yyyy-mm-dd') <= " + Squ(cond.getCreateOrderDateEnd()));
			}
			multiCount++;
		}
